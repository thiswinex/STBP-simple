...
lif_cpp = load(name="lif_cuda", sources=["lif_cuda.cpp", "lif_cuda_kernel.cu"], verbose=True)
#help(lif_cpp)
...
class LIFActFunction(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ctx.save_for_backward(input)
        output = lif_cpp.forward(input, args['Vth'], args['tau'], args['steps'])
        return output

    @staticmethod
    def backward(ctx, grad_output):
        grad_input = lif_cpp.backward(grad_output, *ctx.saved_variables)
        return grad_input
